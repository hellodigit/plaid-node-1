  ...
  function(access_token, start_date, end_date, options, cb) {
    ...
    return wrapPromise(P.coroutine(function*() {
      ...
      while (true) {
        ...
        if (transactionsResponse instanceof Error) { // check if error was returned
          throw transactionsResponse
        }

        try {
          transactionsCount += transactionsResponse.transactions.length
        } catch (err) {
          const transactionsResponseKeys = Object.keys(transactionsResponse)
          console.log("getTransactions response has no transactions array", err)
          console.log("getTransactions response keys", transactionsResponseKeys)

          const malformedResponseError = new PlaidMalformedResponseError()
          malformedResponseError.transactionsResponseKeys = transactionsResponseKeys
...
Client.prototype.getAllInstitutions =
  function(cb) {
    var self = this;
    ...
    return wrapPromise(P.coroutine(function*() {
      ...
      var count = 500; // largest allowed value

      while (true) {
        var institutionsResponse = yield self.getInstitutions(count, institutionsCount);
        if (institutionsResponse instanceof Error) { // check if error was returned
          throw institutionsResponse
        }

        institutionsCount += institutionsResponse.institutions.length;
        institutions = R.concat(institutions, institutionsResponse.institutions);
        if (institutionsCount >= institutionsResponse.total) {
          break;
        }
      }
